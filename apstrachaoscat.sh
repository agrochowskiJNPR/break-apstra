...
echo $intf1id
echo $intf2id
#curl -k --location --request PATCH "https://10.28.207.3/api/blueprints/evpn-vqfx_offbox-virtual/cabling-map" --header "AUTHTOKEN: $authtoken" --header "Content-Type: application/json" --data-raw "{ \"links\": [ {  \"endpoints\": [ { \"interface\": {  \"id\": \"$intf1id\", \"if_name\": "xe-0/0/5" } }, { \"interface\": { \"id\": \"$intf2id\" } } ], \"id\": \"spine1<->evpn_esi_001_leaf2[1]\" } ] }"
curl --location --request PATCH "https://$apstraserver/api/blueprints/$bpid/cabling-map" \
    ...
    \"links\": [
      {
        \"endpoints\": [
          {
            \"interface\": {
              \"id\": \"aOtLsoVNM6qc1zHRQ_A\",
              \"if_name\": \"xe-0/0/5\"
            }
          },
          {
            \"interface\": {
              \"id\": \"GKcOhXTj-b1ArBm6DAE\"
            }
          }
        ],
