...
result=`curl -k --location --request PATCH "https://$apstraserver/api/blueprints/$bpid/cabling-map" --header "AUTHTOKEN: $authtoken" --header "Content-Type: application/json" --data-raw " { \"links\": [ { \"endpoints\": [ { \"interface\": { \"id\": \"$intf1id\", \"if_name\": \"xe-0/0/5\" } }, { \"interface\": { \"id\": \"$intf2id\" } } ], \"id\": \"spine1<->evpn_esi_001_leaf2[1]\" } ] }"`
